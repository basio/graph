...
// A connected componets implemetation which is satify the compuation lever to be used to read from the new format
public class ConnectedComponents extends
    ...
    public void compute(
            ...
            if (currentComponent != vertex.getValue().get()) {
                ...
                for (Edge<DoubleWritable, FloatWritable> edge : vertex.getEdges()) {
                    ...
                    if (neighbor.get() > currentComponent) {
                        double cur_compoment=(double)currentComponent;
                        sendMessage(neighbor, new DoubleWritable(cur_compoment));
                    }
                }
            }
        ...
        for (DoubleWritable message : messages) {
            long candidateComponent = (long)((Double)message.get());
        ...
        if (changed) {
            vertex.setValue(new LongWritable(currentComponent));
            sendMessageToAllEdges(vertex, new DoubleWritable(currentComponent));
        }
        vertex.voteToHalt();
    }
}
